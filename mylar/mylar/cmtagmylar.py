...
def run(dirName, nzbName=None, issueid=None, comversion=None, manual=None, filename=None, module=None, manualmeta=False):
    ...
    comictagger_cmd = os.path.join(mylar.mylar.CMTAGGER_PATH, 'comictagger.py')
    logger.fdebug('ComicTagger Path location for internal comictagger.py set to : ' + comictagger_cmd)

    # Force mylar to use cmtagger_path = mylar.mylar.PROG_DIR to force the use of the included lib.
    ...
    try:
        ...
        new_folder = tempfile.mkdtemp(prefix='mylar_', dir=mylar.mylar.CONFIG.CACHE_DIR) #prefix, suffix, dir
        logger.fdebug('New_Folder: %s' % new_folder)
        new_filepath = os.path.join(new_folder, filename)
        logger.fdebug('New_Filepath: %s' % new_filepath)
        if mylar.mylar.CONFIG.FILE_OPTS == 'copy' and manualmeta == False:
    ...
    if mylar.mylar.CONFIG.FILE_OPTS == 'move':
        cbr2cbzoptions = ["-e", "--delete-rar"]
    else:
        cbr2cbzoptions = ["-e"]

    tagoptions = ["-s"]
    if mylar.mylar.CONFIG.CMTAG_VOLUME:
    ...
    if int(ctcheck) >= int('1115'):  # (v1.1.15)
        if any([mylar.mylar.CONFIG.COMICVINE_API == 'None', mylar.mylar.CONFIG.COMICVINE_API is None]):
            logger.fdebug(module + ' ' + ctversion[:ctend] + ' being used - no personal ComicVine API Key supplied. Take your chances.')
            use_cvapi = "False"
        else:
            logger.fdebug(module + ' ' + ctversion[:ctend] + ' being used - using personal ComicVine API key supplied via mylar.mylar.')
            use_cvapi = "True"
            tagoptions.extend(["--cv-api-key", mylar.mylar.CONFIG.COMICVINE_API])
    else:
    ...
    if mylar.mylar.CONFIG.CT_TAG_CR:
        tagcnt = 1
        logger.fdebug(module + ' CR Tagging enabled.')

    if mylar.mylar.CONFIG.CT_TAG_CBL:
        if not mylar.mylar.CONFIG.CT_TAG_CR: i = 2  #set the tag to start at cbl and end without doing another tagging.
        tagcnt = 2
    ...
    if filename.endswith('.cbz'):
        if mylar.mylar.CONFIG.CT_CBZ_OVERWRITE:
    ...
    while (i <= tagcnt):
        ...
        if initial_ctrun:
            ...
            logger.fdebug(module + ' Enabling ComicTagger script: ' + str(currentScriptName) + ' with options: ' + re.sub(f_tagoptions[f_tagoptions.index(mylar.mylar.CONFIG.COMICVINE_API)], 'REDACTED', str(f_tagoptions)))
            # generate a safe command line string to execute the script and provide all the parameters
            script_cmdlog = re.sub(f_tagoptions[f_tagoptions.index(mylar.mylar.CONFIG.COMICVINE_API)], 'REDACTED', str(script_cmd))
        ...
        try:
            ...
            if initial_ctrun and 'exported successfully' in out:
                ...
                if 'file is not expected size' in out:
                    ...
                    logger.warn(module + '[COMIC-TAGGER][CBR-TO-CBZ] Failed to convert cbr to cbz - check permissions on folder : ' + mylar.mylar.CONFIG.CACHE_DIR + ' and/or the location where Mylar is trying to tag the files from.')
                    tidyup(og_filepath, new_filepath, new_folder, manualmeta)
                    return 'fail'
            ...
            logger.warn(module + '[COMIC-TAGGER] Unable to run comictagger with the options provided: ' + re.sub(f_tagoptions[f_tagoptions.index(mylar.mylar.CONFIG.COMICVINE_API)], 'REDACTED', str(script_cmd)))
            tidyup(filepath, new_filepath, new_folder, manualmeta)
            return "fail"

        if mylar.mylar.CONFIG.CBR2CBZ_ONLY and initial_ctrun == False:
            break
