...
from mylar.mylar.auth import AuthController, require
...
def serve_template(templatename, **kwargs):
    interface_dir = os.path.join(str(mylar.mylar.PROG_DIR), 'data/interfaces/')
    template_dir = os.path.join(str(interface_dir), mylar.mylar.CONFIG.INTERFACE)
    _hplookup = TemplateLookup(directories=[template_dir])
    try:
        template = _hplookup.get_template(templatename)
        return template.render(http_root=mylar.mylar.CONFIG.HTTP_ROOT, **kwargs)
    except:
...
>>>>>>> Make Mylar launch under Python 3
    ...
    def comicDetails(self, ComicID):
        ...
        skipno = len(mylar.mylar.COMICSORT['SortOrder'])
        lastno = mylar.mylar.COMICSORT['LastOrderNo']
        lastid = mylar.mylar.COMICSORT['LastOrderID']
        series = {}
        ...
        i = 0
        while (i < skipno):
            cskip = mylar.mylar.COMICSORT['SortOrder'][i]
            if cskip['ComicID'] == ComicID:
                ...
                if cursortnum == 0:
                    ...
                    previous = mylar.mylar.COMICSORT['SortOrder'][i - 1]['ComicID']

                # if last record, set the Next record to the FIRST record.
                if cursortnum == lastno:
                    next = mylar.mylar.COMICSORT['SortOrder'][0]['ComicID']
                else:
                    next = mylar.mylar.COMICSORT['SortOrder'][i + 1]['ComicID']
        ...
        if force_continuing is None:
            force_continuing = 0
        if mylar.mylar.CONFIG.DELETE_REMOVE_DIR is None:
            mylar.mylar.CONFIG.DELETE_REMOVE_DIR = 0
        if allowpacks is None:
            allowpacks = "0"
        if all([comic['Corrected_SeriesYear'] is not None, comic['Corrected_SeriesYear'] != '',
        ...
        #        imageurl = mylar.mylar.cv.getComic(comic['ComicID'], 'image', issueid=imagetopull[0])
        ...
        if mylar.CONFIG.ANNUALS_ON:
            ...
            annuals_list = None
            aName = None
        return serve_template(templatename="comicdetails.html", title=comic['ComicName'], comic=comic, issues=issues,
                              comicConfig=comicConfig, isCounts=isCounts, series=series, annuals=annuals_list,
                              annualinfo=aName)
    ...
    def searchit(self, name, issue=None, mode=None, type=None, serinfo=None):
        if type is None:
            ...
            logger.fdebug(str(type) + " mode enabled.")
        ...
        if type == 'comic' and mode == 'pullseries':
            ...
            try:
                ...
                logger.error(
                    'Unable to perform required pull-list search for : [name: ' + name + '][issue: ' + issue + '][mode: ' + mode + ']')
                return
        elif type == 'comic' and mode == 'series':
            if name.startswith('4050-'):
                mismatch = "no"
                comicid = re.sub('4050-', '', name)
                logger.info('Attempting to add directly by ComicVineID: ' + str(
                    comicid) + '. I sure hope you know what you are doing.')
            ...
            try:
                ...
                logger.error(
                    'Unable to perform required one-off pull-list search for : [name: ' + name + '][issue: ' + issue + '][mode: ' + mode + ']')
                return
        elif type == 'story_arc':
            try:
    ...
    def addComic(self, comicid, comicname=None, comicyear=None, comicimage=None, comicissues=None, comicpublisher=None,
                 imported=None, ogcname=None, serinfo=None):
        myDB = db.DBConnection()
        if imported == "confirm":
        ...
        if not mylar.mylar.CONFIG.CV_ONLY:
            ...
            if CV_EXcomicid is None:  # pass #
                ...
                if gcdinfo == "No Match":
                    ...
                    cvdata = mylar.mylar.cv.getComic(comicid, 'comic')
                    comicimage = cvdata['ComicImage']
                    updater.no_searchresults(comicid)
                    nomatch = "true"
                    u_comicname = comicname.encode('utf-8').strip()
                    logger.info("I couldn't find an exact match for " + u_comicname + " (" + str(
                        comicyear) + ") - gathering data for Error-Checking screen (this could take a minute)...")
                    i = 0
                    ...
                    if imported != 'None':
                        ...
                        return serve_template(templatename="searchfix.html", title="Error Check", comicname=comicname,
                                              comicid=comicid, comicyear=comicyear, comicimage=comicimage,
                                              comicissues=comicissues, cresults=cresults, imported=None, ogcname=None)
                else:
                    nomatch = "false"
                ...
                if CV_EXcomicid['NewComicID'] == 'none':
                    ...
                    for exc in testspx:
                        ...
                        return serve_template(templatename="searchfix-2.html", title="In-Depth Results",
                                              sresults=sresults)
        ...
        time.sleep(
            5)  # wait 5s so the db can be populated enough to display the page - otherwise will return to home page if not enough info is loaded.
        raise cherrypy.HTTPRedirect("comicDetails?ComicID=%s" % comicid)
    ...
    def addStoryArc(self, arcid, arcrefresh=False, cvarcid=None, arclist=None, storyarcname=None, storyarcyear=None,
                    storyarcpublisher=None, storyarcissues=None, desc=None, image=None):
        ...
        if arc_chk is None:
            if arcrefresh:
                logger.warn(
                    module + ' Unable to retrieve Story Arc ComicVine ID from the db. Unable to refresh Story Arc at this time. You probably have to delete/readd the story arc this one time for Refreshing to work properly.')
                return
            else:
                logger.fdebug(
            ...
            if arcrefresh:  # cvarcid must be present here as well..
                logger.info(module + '[' + str(arcid) + '] Successfully found Story Arc ComicVine ID [4045-' + str(
                    cvarcid) + '] within db. Preparing to refresh Story Arc.')
        ...
        if not os.path.isdir(os.path.join(mylar.mylar.CONFIG.CACHE_DIR, 'storyarcs')):
            checkdirectory = filechecker.validateAndCreateDirectory(os.path.join(mylar.mylar.CONFIG.CACHE_DIR, 'storyarcs'),
                                                                    True)
        ...
        coverfile = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, 'storyarcs', str(cvarcid) + "-banner.jpg")
        ...
        arc_results = mylar.mylar.cv.getComic(comicid=None, type='issue', arcid=arcid, arclist=arclist)
    ...
    def from_Exceptions(self, comicid, gcdid, comicname=None, comicyear=None, comicissues=None, comicpublisher=None,
        ...
        except_file = os.path.join(mylar.mylar.DATA_DIR, "custom_exceptions.csv")
        if not os.path.exists(except_file):
            try:
                csvfile = open(str(except_file), 'rb')
                csvfile.close()
            except (OSError, IOError):
                logger.error("Could not locate " + str(
                    except_file) + " file. Make sure it's in datadir: " + mylar.mylar.DATA_DIR + " with proper permissions.")
                return
    ...
    def GCDaddComic(self, comicid, comicname=None, comicyear=None, comicissues=None, comiccover=None,
                    comicpublisher=None):
    ...
    def post_process(self, nzb_name, nzb_folder, failed=False, apc_version=None, comicrn_version=None):
        if all([nzb_name != 'Manual Run', nzb_name != 'Manual+Run']):
            if comicrn_version is None and apc_version is None:
                logger.warn('ComicRN should be v' + str(
                    mylar.mylar.STATIC_COMICRN_VERSION) + ' and autoProcessComics.py should be v' + str(
                    mylar.mylar.STATIC_APC_VERSION) + ', but they are not and are out of date. Post-Processing may or may not work.')
            elif comicrn_version is None or comicrn_version != mylar.mylar.STATIC_COMICRN_VERSION:
                if comicrn_version == 'None':
                    comicrn_version = "0"
                logger.warn(
                    'Your ComicRN.py script should be v' + str(mylar.mylar.STATIC_COMICRN_VERSION) + ', but is v' + str(
                        comicrn_version) + ' and is out of date. Things may still work - but you are taking your chances.')
            elif apc_version is None or apc_version != mylar.mylar.STATIC_APC_VERSION:
                if apc_version == 'None':
                    apc_version = "0"
                if mylar.mylar.CONFIG.AUTHENTICATION == 2:
                    logger.warn(
                ...
                logger.warn(
                    'Your autoProcessComics.py script should be v' + str(mylar.mylar.STATIC_APC_VERSION) + ', but is v' + str(
                        apc_version) + ' and is out of date. Odds are something is gonna fail - you should update it.')
            else:
                logger.info('ComicRN.py version: ' + str(comicrn_version) + ' -- autoProcessComics.py version: ' + str(
                    apc_version))
        ...
        if failed:
            if mylar.mylar.CONFIG.FAILED_DOWNLOAD_HANDLING is True:
                ...
                if failchk[0]['mode'] == 'retry':
                    ...
                    if failchk[0]['annchk'] == 'no':
                        ...
                        mode = 'want_ann'
                    self.queueit(mode=mode, ComicName=failchk[0]['comicname'], ComicIssue=failchk[0]['issuenumber'],
                                 ComicID=failchk[0]['comicid'], IssueID=failchk[0]['issueid'], manualsearch=True)
    ...
    def deleteSeries(self, ComicID, delete_dir=None):
        ...
        if delete_dir:  # mylar.mylar.CONFIG.DELETE_REMOVE_DIR:
    ...
    def force_rss(self):
        ...
        forcethis = mylar.mylar.rsscheckit.tehMain()
    ...
    def markissues(self, action=None, **args):
        ...
        for IssueID in args:
            if any([IssueID is None, 'issue_table' in IssueID, 'history_table' in IssueID, 'manage_issues' in IssueID,
                    'issue_table_length' in IssueID, 'issues' in IssueID, 'annuals' in IssueID,
                ...
                if action == 'Downloaded':
                    if mi['Status'] == "Skipped" or mi['Status'] == "Wanted":
                        logger.fdebug(
                            u"Cannot change status to %s as comic is not Snatched or Downloaded" % (newaction))
                        continue
                ...
                elif action == 'Failed' and mylar.mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
                    logger.fdebug('Marking [' + comicname + '] : ' + str(
                        IssueID) + ' as Failed. Sending to failed download handler.')
        ...
        if action == 'Failed' and mylar.mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
    ...
    def markentries(self, action=None, **args):
        ...
        for ID in args:
            ...
            if any([ID is None, 'manage_failed_length' in ID]):
                ...
                cnt += 1
        logger.info('[DB FAILED CLEANSING] Cleared ' + str(
            cnt) + ' entries from the Failed DB so they will now be downloaded if available/working.')
    ...
    def retryissue(self, ComicName, ComicID, IssueID, IssueNumber, ReleaseComicID=None, ComicYear=None, redirect=None):
        ...
        for cs in chk_snatch:
            if cs['Provider'] == 'CBT' or cs['Provider'] == 'KAT':
                logger.info('Invalid provider attached to download (' + cs[
                    'Provider'] + '). I cannot find this on 32P, so ignoring this result.')
        ...
        for ps in sorted(providers_snatched, key=itemgetter('DateAdded', 'Provider'), reverse=True):
            try:
                ...
                chk_the_log = myDB.selectone('SELECT * FROM nzblog WHERE IssueID=? AND Provider like (?)',
                                             [IssueID, Provider_sql]).fetchone()
            except:
                logger.warn(
                ...
                chk_the_log = myDB.selectone(
                    'SELECT * FROM nzblog WHERE IssueID=? and Provider != "CBT" and Provider != "KAT"',
                    [IssueID]).fetchone()

            if chk_the_log is None:
                if len(providers_snatched) == 1:
                    logger.info('Unable to locate provider information ' + ps[
                        'Provider'] + ' from nzblog - if you wiped the log, you have to search/download as per normal')
                    return
                else:
                    logger.info('Unable to locate provider information ' + ps[
                        'Provider'] + ' from nzblog. Checking additional providers that came back as being used to download this issue')
                    continue
            else:
        ...
        if all([ComicYear is not None, ComicYear != 'None']) and all([IssueID is not None, IssueID != 'None']):
            ...
            if getYear is None:
                logger.warn(
                    'Unable to retrieve valid Issue Date for Retry of Issue (Try to refresh the series and then try again.')
                return
        ...
        for chk_log in chk_logresults:
            ...
            if any([fullprov == '32P', fullprov == 'WWT', fullprov == 'DEM']):
                ...
                rcheck = mylar.mylar.rsscheck.torsend2client(ComicName, IssueNumber, ComicYear, id, fullprov)
                if rcheck == "fail":
                    logger.error("Unable to send torrent - check logs and settings.")
                    continue
                else:
                    if any([mylar.mylar.USE_RTORRENT, mylar.mylar.USE_DELUGE]) and mylar.mylar.CONFIG.AUTO_SNATCH:
                        mylar.mylar.SNATCHED_QUEUE.put(rcheck['hash'])
                    elif mylar.mylar.CONFIG.ENABLE_SNATCH_SCRIPT:
                ...
                chkthis = myDB.selectone(
                    'SELECT a.ComicID, a.ComicName, a.ComicVersion, a.ComicYear, b.IssueID, b.Issue_Number, b.IssueDate FROM comics as a INNER JOIN annuals as b ON a.ComicID = b.ComicID WHERE IssueID=?',
                    [IssueID]).fetchone()
                if chkthis is None:
                    chkthis = myDB.selectone(
                    ...
                    if chkthis is None:
                        ...
                        if chkthis is None:
                            logger.warn(
                                'Unable to locate previous snatch details (checked issues/annuals/one-offs). Retrying the snatch for this issue is unavailable.')
                            continue
                        else:
                            logger.fdebug(
                ...
                if fullprov == 'nzb.su':
                    if not mylar.mylar.CONFIG.NZBSU:
                        logger.error(
                            'nzb.su is not enabled - unable to process retry request until provider is re-enabled.')
                        continue
                    # http://nzb.su/getnzb/ea1befdeee0affd663735b2b09010140.nzb&i=<uid>&r=<passkey>
                    link = 'http://nzb.su/getnzb/' + str(id) + '.nzb&i=' + str(mylar.mylar.CONFIG.NZBSU_UID) + '&r=' + str(
                        mylar.mylar.CONFIG.NZBSU_APIKEY)
                    logger.info('fetched via nzb.su. Retrying the send : ' + str(link))
                elif fullprov == 'dognzb':
                    if not mylar.mylar.CONFIG.DOGNZB:
                        logger.error(
                            'Dognzb is not enabled - unable to process retry request until provider is re-enabled.')
                        continue
                    # https://dognzb.cr/fetch/5931874bf7381b274f647712b796f0ac/<passkey>
                    link = 'https://dognzb.cr/fetch/' + str(id) + '/' + str(mylar.mylar.CONFIG.DOGNZB_APIKEY)
                    logger.info('fetched via dognzb. Retrying the send : ' + str(link))
                elif fullprov == 'experimental':
                    if not mylar.mylar.CONFIG.EXPERIMENTAL:
                        logger.error(
                    ...
                    if not mylar.mylar.CONFIG.NEWZNAB:
                        logger.error(
                    ...
                    for newznab_info in mylar.mylar.CONFIG.EXTRA_NEWZNABS:
                        if tmpprov.lower() in newznab_info[0].lower():
                            if (newznab_info[5] == '1' or newznab_info[5] == 1):
                                ...
                                newznabinfo = (
                                newznab_info[0], newznab_info[1], newznab_info[2], newznab_info[3], newznab_info[4])
                            else:
                                logger.error(str(newznab_info[
                                                     0]) + ' is not enabled - unable to process retry request until provider is re-enabled.')
                            break

                if link is not None:
                    sendit = search.searcher(fullprov, nzbname, comicinfo, link=link, IssueID=IssueID, ComicID=ComicID,
                                             tmpprov=fullprov, directsend=True, newznab=newznabinfo)
                    break
        return
    ...
    def queueissue(self, mode, ComicName=None, ComicID=None, ComicYear=None, ComicIssue=None, IssueID=None, new=False,
                   redirect=None, SeriesYear=None, SARC=None, IssueArcID=None, manualsearch=None, Publisher=None,
                   pullinfo=None, pullweek=None, pullyear=None, manual=False, ComicVersion=None):
        logger.fdebug('ComicID:' + str(ComicID))
        ...
        if ComicID is None and mode == 'series':
            ...
            if mode == 'want':
                ...
                annual_name = myDB.selectone("SELECT * FROM annuals WHERE ComicID=? and IssueID=?",
...
#                return updater.foundsearch(ComicID, IssueID, mode=mode, provider=prov, hash=foundcom['info']['t_hash'])
    ...
    def unqueueissue(self, IssueID, ComicID, ComicName=None, Issue=None, FutureID=None, mode=None, ReleaseComicID=None):
        ...
        if ComicName is None:
            ...
            if issue is None:
                if mylar.mylar.CONFIG.ANNUALS_ON:
                    if ReleaseComicID is None:
                        ...
                        issann = myDB.selectone('SELECT * FROM annuals WHERE IssueID=? AND ReleaseComicID=?',
                                                [IssueID, ReleaseComicID]).fetchone()
            ...
            controlValueDict = {"IssueID": IssueID}
            if mode == 'failed' and mylar.mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
            ...
            if thefuture is None:
                logger.info(
                    'Cannot find the corresponding issue in the Futures List for some reason. This is probably an Error.')
            else:
    ...
    def pullSearch(self, week, year):
        ...
        if len(issuesToSearch) > 0:
            logger.info(
                'Now force searching for ' + str(len(issuesToSearch)) + ' issues from the pullist for week ' + str(
                    week))
    ...
    def pullist(self, week=None, year=None, generateonly=False):
        ...
        if popit:
            w_results = myDB.select("SELECT * from weekly WHERE weeknumber=? AND year=?",
            ...
            if len(w_results) == 0:
                ...
                if repoll['status'] == 'success':
                    ...
                    logger.warn('Problem repopulating the pullist for week ' + str(weekinfo['weeknumber']) + ', ' + str(
                        weekinfo['year']))
            ...
            if all([w_results is None, generateonly is False]):
                return serve_template(templatename="weeklypull.html", title="Weekly Pull", weeklyresults=weeklyresults,
                                      pullfilter=True, weekfold=weekinfo['week_folder'], wantedcount=0,
                                      weekinfo=weekinfo)
            ...
            for weekly in w_results:
                ...
                if weekly['ComicID'] in watchlibrary:
                    ...
                    if weekinfo['weeknumber']:
                        if watchlibrary[weekly['ComicID']]['status'] == 'Paused':
                            tmp_status = 'Paused'
                        elif any([week >= int(weekinfo['weeknumber']), week is None]) and all(
                                [mylar.mylar.CONFIG.AUTOWANT_UPCOMING, tmp_status == 'Skipped']):
                            tmp_status = 'Wanted'
                ...
                try:
                    ...
                    if 'au' in weekly['ISSUE'].lower() or 'ai' in weekly['ISSUE'].lower() or '.inh' in weekly[
                        'ISSUE'].lower() or '.now' in weekly['ISSUE'].lower() or '.mu' in weekly['ISSUE'].lower():
                        x = weekly['ISSUE']
        ...
        if generateonly is True:
            ...
            if week:
                return serve_template(templatename="weeklypull.html", title="Weekly Pull", weeklyresults=weeklyresults,
                                      pullfilter=True, weekfold=weekinfo['week_folder'], wantedcount=wantedcount,
                                      weekinfo=weekinfo)
            else:
                return serve_template(templatename="weeklypull.html", title="Weekly Pull", weeklyresults=weeklyresults,
                                      pullfilter=True, weekfold=weekinfo['week_folder'], wantedcount=wantedcount,
    ...
    def removeautowant(self, comicname, release):
        ...
        myDB.action("DELETE FROM futureupcoming WHERE ComicName=? AND IssueDate=? AND Status='Wanted'",
                    [comicname, release])

    removeautowant.exposed = True

    def futurepull(self):
        from mylar.mylar.mylar import solicit
    ...
    def futurepulllist(self):
        ...
        if popthis:
            ...
            logger.fdebug('There are ' + str(
                len(watchresults)) + ' issues that you are watching for but are not on your watchlist yet.')

        popit = myDB.select("SELECT * FROM sqlite_master WHERE name='future' and type='table'")
        if popit:
            f_results = myDB.select(
                "SELECT SHIPDATE, PUBLISHER, ISSUE, COMIC, EXTRA, STATUS, ComicID, FutureID from future")
            for future in f_results:
                ...
                try:
                    ...
                    if 'au' in future['ISSUE'].lower() or 'ai' in future['ISSUE'].lower() or '.inh' in future[
                        'ISSUE'].lower() or '.now' in future['ISSUE'].lower() or '.mu' in future['ISSUE'].lower():
                        x = future['ISSUE']
            ...
            return
        return serve_template(templatename="futurepull.html", title="future Pull", futureresults=futureresults,
                              pullfilter=True)
    ...
    def add2futurewatchlist(self, ComicName, Issue, Publisher, ShipDate, weeknumber, year, FutureID=None):
        ...
        if FutureID is not None:
            chkfuture = myDB.selectone(
                'SELECT * FROM futureupcoming WHERE ComicName=? AND IssueNumber=? WHERE weeknumber=? AND year=?',
                [ComicName, Issue, weeknumber, year]).fetchone()
    ...
    def filterpull(self):
        ...
        if pulldate is None:
            raise cherrypy.HTTPRedirect("home")
        return serve_template(templatename="weeklypull.html", title="Weekly Pull", weeklyresults=weeklyresults,
                              pulldate=pulldate['SHIPDATE'], pullfilter=True)

    filterpull.exposed = True

    def manualpull(self, weeknumber=None, year=None):
        logger.info(
            'ALT_PULL: ' + str(mylar.mylar.CONFIG.ALT_PULL) + ' PULLBYFILE: ' + str(mylar.mylar.PULLBYFILE) + ' week: ' + str(
                weeknumber) + ' year: ' + str(year))
        if all([mylar.mylar.CONFIG.ALT_PULL == 2, mylar.mylar.PULLBYFILE is False]) and weeknumber:
            return mylar.mylar.locg.locg(weeknumber=weeknumber, year=year)
    ...
    def pullrecreate(self, weeknumber=None, year=None):
        ...
        if weeknumber is None:
            ...
            logger.info(
                "Deleted existing pull-list data for week %s, %s. Now Recreating the Pull-list..." % (weeknumber, year))
    ...
    def upcoming(self):
        ...
        upcomingdata = myDB.select(
            "SELECT * from weekly WHERE Issue is not NULL AND Comic is not NULL order by weeknumber DESC")
        if upcomingdata is None:
            ...
            for upc in upcomingdata:
                ...
                mylar.mylar.WANTED_TAB_OFF = False
                try:
                    ...
                    mylar.mylar.WANTED_TAB_OFF = True
                    myDB.action("DROP TABLE weekly")
                    mylar.mylar.dbcheck()
        ...
        if mylar.mylar.CONFIG.UPCOMING_STORYARCS is True:
            arcs = myDB.select("SELECT * from storyarcs WHERE Status='Wanted'")
        else:
            arcs = []
        if mylar.mylar.CONFIG.UPCOMING_SNATCHED is True:
...
>>>>>>> Make Mylar launch under Python 3
        ...
        if mylar.mylar.CONFIG.ANNUALS_ON:
            ...
            if mylar.mylar.CONFIG.UPCOMING_SNATCHED:
                annuals_list += myDB.select("SELECT * FROM annuals WHERE Status='Snatched'")
            if mylar.mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
...
>>>>>>> Make Mylar launch under Python 3
        ...
        for mvup in mvupcome:
            myissue = myDB.selectone("SELECT ComicName, Issue_Number, IssueID, ComicID FROM issues WHERE IssueID=?",
                                     [mvup['IssueID']]).fetchone()
            ...
            if myissue is None:
                ...
                deleteit = myDB.action("DELETE from upcoming WHERE ComicName=? AND IssueNumber=?",
                                       [mvup['ComicName'], mvup['IssueNumber']])
    ...
    def previewRename(self, comicidlist):
        ...
        for comicid in comicidlist:
            ...
            comicname = comic['ComicName']
            issue = myDB.selectone("SELECT * FROM issues WHERE ComicID=? AND Location is not None ORDER BY ReleaseDate",
                                   [comicid]).fetchone()
            if 'annual' in issue['Location'].lower():
                annualize = 'yes'
            else:
                annualize = None
            renameiss = helpers.rename_param(comicid, comicname, issue['Issue_Number'], issue['Location'],
                                             comicyear=None, issueid=issue['IssueID'], annualize=annualize)
    ...
    def manualRename(self, comicid):
        ...
        for cid in comiclist:
            ...
            try:
                if mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS is not None and mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS != 'None' and os.path.join(
                        mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS, os.path.basename(comicdir)) != comicdir:
                    logger.fdebug('multiple_dest_dirs:' + mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS)
                    logger.fdebug('dir: ' + comicdir)
                    logger.fdebug('os.path.basename: ' + os.path.basename(comicdir))
                    pathdir = os.path.join(mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS, os.path.basename(comicdir))
            except:
                pass
                        ...
                        for issue in issues:
                            if issue['Location'] == filename:
                                ...
                                if 'annual' in issue['Location'].lower():
                                    ...
                                    annualize = None
                                renameiss = helpers.rename_param(cid, comicname, issue['Issue_Number'], filename,
                                                                 comicyear=comicyear, issueid=issue['IssueID'],
                                                                 annualize=annualize)
    ...
    def manage(self):
        mylarRoot = mylar.mylar.CONFIG.DESTINATION_DIR
        from . import db
        myDB = db.DBConnection()
        ...
        if jobresults is not None:
            ...
            for jb in jobresults:
                if jb['prev_run_datetime'] is not None:
                    try:
                        pr = (datetime.datetime.strptime(jb['prev_run_datetime'][:19],
                                                         '%Y-%m-%d %H:%M:%S') - datetime.datetime.utcfromtimestamp(
                            0)).total_seconds()
                    except ValueError:
                        pr = (datetime.datetime.strptime(jb['prev_run_datetime'],
                                                         '%Y-%m-%d %H:%M:%S.%f') - datetime.datetime.utcfromtimestamp(
                            0)).total_seconds()
                ...
                if jb['next_run_datetime'] is not None:
                    try:
                        nr = (datetime.datetime.strptime(jb['next_run_datetime'][:19],
                                                         '%Y-%m-%d %H:%M:%S') - datetime.datetime.utcfromtimestamp(
                            0)).total_seconds()
                    except ValueError:
                        nr = (datetime.datetime.strptime(jb['next_run_datetime'],
                                                         '%Y-%m-%d %H:%M:%S.%f') - datetime.datetime.utcfromtimestamp(
                            0)).total_seconds()
                ...
                if 'rss' in jb['JobName'].lower():
                    status = mylar.mylar.RSS_STATUS
                    interval = str(mylar.mylar.CONFIG.RSS_CHECKINTERVAL) + ' mins'
                if 'weekly' in jb['JobName'].lower():
                    status = mylar.mylar.WEEKLY_STATUS
                    if mylar.mylar.CONFIG.ALT_PULL == 2:
                        interval = '4 hrs'
                    else:
                        interval = '24 hrs'
                if 'search' in jb['JobName'].lower():
                    status = mylar.mylar.SEARCH_STATUS
                    interval = str(mylar.mylar.CONFIG.SEARCH_INTERVAL) + ' mins'
                if 'updater' in jb['JobName'].lower():
                    status = mylar.mylar.UPDATER_STATUS
                    interval = str(int(mylar.mylar.DBUPDATE_INTERVAL)) + ' mins'
                if 'folder' in jb['JobName'].lower():
                    status = mylar.mylar.MONITOR_STATUS
                    interval = str(mylar.mylar.CONFIG.DOWNLOAD_SCAN_INTERVAL) + ' mins'
                if 'version' in jb['JobName'].lower():
                    status = mylar.mylar.VERSION_STATUS
                    interval = str(mylar.mylar.CONFIG.CHECK_GITHUB_INTERVAL) + 'mins'
    ...
    def jobmanage(self, job, mode):
        ...
        jobid = None
        job_id_map = {'DB Updater': 'dbupdater', 'Auto-Search': 'search', 'RSS Feeds': 'rss',
                      'Weekly Pullist': 'weekly', 'Check Version': 'version', 'Folder Monitor': 'monitor'}
    ...
    def schedulerForceCheck(self, jobid):
        ...
        for jb in mylar.mylar.SCHED.get_jobs():
            if jobid.lower() in str(jb).lower():
                ...
                if any([jobid == 'rss', jobid == 'weekly', jobid == 'search', jobid == 'version', jobid == 'updater',
                        jobid == 'monitor']):
    ...
    def manageIssues(self, **kwargs):
        ...
        return serve_template(templatename="manageissues.html", title="Manage " + str(status) + " Issues",
                              issues=results, status=status)
    ...
    def markComics(self, action=None, **args):
        ...
        for cl in clist:
            if action == 'delete':
                logger.info('[MANAGE COMICS][DELETION] Now deleting ' + cl['ComicName'] + ' (' + str(
                    cl['ComicYear']) + ') [' + str(cl['ComicID']) + '] form the DB.')
                ...
                logger.info('[MANAGE COMICS][DELETION] Successfully deleted ' + cl['ComicName'] + '(' + str(
                    cl['ComicYear']) + ')')
            elif action == 'pause':
    ...
    def forceUpdate(self):
        from mylar.mylar.mylar import updater
    ...
    def forceSearch(self):
        # from mylar.mylar.mylar import search
    ...
    def forceRescan(self, ComicID, bulk=False, action='recheck'):
        if bulk:
            ...
            if action == 'recheck':
                for cid in ComicID:
                    logger.info(
                        '[MASS BATCH][RECHECK-FILES][' + str(cnt) + '/' + str(len(ComicID)) + '] Rechecking ' + cid[
                            'ComicName'] + '(' + str(cid['ComicYear']) + ')')
                    updater.forceRescan(cid['ComicID'])
                    cnt += 1
                logger.info('[MASS BATCH][RECHECK-FILES] I have completed rechecking files for ' + str(
                    len(ComicID)) + ' series.')
            else:
                for cid in ComicID:
                    logger.info('[MASS BATCH][METATAGGING-FILES][' + str(cnt) + '/' + str(
                        len(ComicID)) + '] Now Preparing to metatag series for ' + cid['ComicName'] + '(' + str(
                        cid['ComicYear']) + ')')
                    self.group_metatag(ComicID=cid['ComicID'])
                    cnt += 1
                logger.info('[MASS BATCH][METATAGGING-FILES] I have completed metatagging files for ' + str(
                    len(ComicID)) + ' series.')
    ...
    def checkGithub(self):
        from mylar.mylar.mylar import versioncheck
    ...
    def storyarc_main(self, arcid=None):
        ...
        if arcid is None:
            alist = myDB.select(
                "SELECT * from storyarcs WHERE ComicName is not Null GROUP BY StoryArcID")  # COLLATE NOCASE")
        else:
            alist = myDB.select(
                "SELECT * from storyarcs WHERE ComicName is not Null AND StoryArcID=? GROUP BY StoryArcID",
                [arcid])  # COLLATE NOCASE")
    ...
    def detailStoryArc(self, StoryArcID, StoryArcName=None):
        ...
        try:
            ...
            cvarcid = None
            sdir = mylar.mylar.CONFIG.GRABBAG_DIR
        ...
        return serve_template(templatename=template, title="Detailed Arc list", readlist=arcinfo,
                              storyarcname=StoryArcName, storyarcid=StoryArcID, cvarcid=cvarcid, sdir=sdir,
                              arcdetail=arcdetail, storyarcbanner=storyarcbanner, bannerheight=bannerheight,
                              bannerwidth=bannerwidth, spanyears=spanyears)
    ...
    def order_edit(self, id, value):
        ...
        myDB = db.DBConnection()
        readchk = myDB.select(
            "SELECT * FROM storyarcs WHERE StoryArcID=? AND NOT Manual is 'deleted' ORDER BY ReadingOrder",
            [storyarcid])
        if readchk is None:
        ...
        for rc in sorted(readchk, key=itemgetter('ReadingOrder'), reverse=False):
            if str(issuearcid) == str(rc['IssueArcID']):
                ...
                issueid = rc['IssueID']
                IssueArcID = rc['IssueArcID']
                logger.fdebug('reordering existing sequence as lower sequence has changed. Altering from %s to %s' % (
    ...
    def importReadlist(self, filename):
        ...
        if Arc_MultipleSeries is None:
            ...
            for MSCheck in AMS:
                thischk = myDB.select('SELECT * FROM storyarcs WHERE ComicName=? AND SeriesYear=?',
        ...
        for duh in AMS:
            ...
            if len(sresults) == 1:
                sr = sresults[0]
                logger.info('Only one result...automagik-mode enabled for ' + duh['ComicName'] + ' :: ' + str(
                    sr['comicid']) + ' :: Publisher : ' + str(sr['publisher']))
                issues = mylar.mylar.cv.getComic(sr['comicid'], 'issue')
                isscnt = len(issues['issuechoice'])
                logger.info('isscnt : ' + str(isscnt))
                chklist = myDB.select('SELECT * FROM storyarcs WHERE StoryArcID=? AND ComicName=? AND SeriesYear=?',
                    ...
                    while (n <= isscnt):
                        ...
                        for d in chklist:
                            if islval['Issue_Number'] == d['IssueNumber']:
                                logger.info('[' + str(islval['Issue_ID']) + '] matched on Issue Number for ' + duh[
                                    'ComicName'] + ' #' + str(d['IssueNumber']))
                                logger.info('I should write these dates: ' + islval['Issue_Date'] + ' -- ' + islval[
                                    'Store_Date'])
    ...
    def ArcWatchlist(self, StoryArcID=None):
        ...
        if ArcWatch is None:
            ...
            if dstloc is not None:
                if not os.path.isdir(dstloc):
                    if mylar.mylar.CONFIG.STORYARCDIR:
                        ...
                        logger.info(
                            'Story Arc Grab-Bag Directory [' + dstloc + '] does not exist! - attempting to create now.')
                    checkdirectory = filechecker.validateAndCreateDirectory(dstloc, True)
                    if not checkdirectory:
                        logger.warn('Error trying to validate/create directory. Aborting this process at this time.')
                        return

                if all([mylar.mylar.CONFIG.CVINFO, mylar.mylar.CONFIG.STORYARCDIR]):
                    if not os.path.isfile(os.path.join(dstloc, "cvinfo")) or mylar.mylar.CONFIG.CV_ONETIMER:
                        logger.fdebug('Generating cvinfo file for story-arc.')
                        with open(os.path.join(dstloc, "cvinfo"), "w") as text_file:
                            if any([ArcWatch[0]['StoryArcID'] == ArcWatch[0]['CV_ArcID'],
                                    ArcWatch[0]['CV_ArcID'] is None]):
                ...
                if mylar.mylar.CONFIG.STORYARCDIR:
                    fchk = filechecker.FileChecker(dir=dstloc, watchcomic=None, Publisher=None, sarc='true',
                                                   justparse=True)
            ...
            for arc in ArcWatch:
                ...
                comics = myDB.select("SELECT * FROM comics WHERE DynamicComicName IN (?) COLLATE NOCASE",
                                     [arc['DynamicComicName']])

                for comic in comics:
                    mod_watch = comic['DynamicComicName']  # is from the comics db

                    if re.sub('[\|\s]', '', mod_watch.lower()).strip() == re.sub('[\|\s]', '', arc[
                        'DynamicComicName'].lower()).strip():
                        ...
                        if comic['ComicID'][:1] == 'G':
                            ...
                            isschk = myDB.selectone("SELECT * FROM issues WHERE Issue_Number=? AND ComicID=?",
                                                    [str(GCDissue), comic['ComicID']]).fetchone()
                        else:
                            issue_int = helpers.issuedigits(arc['IssueNumber'])
                            logger.fdebug('int_issue = ' + str(issue_int))
                            isschk = myDB.selectone("SELECT * FROM issues WHERE Int_IssueNumber=? AND ComicID=?",
                                                    [issue_int, comic[
                                                        'ComicID']]).fetchone()  # AND STATUS !='Snatched'", [issue_int, comic['ComicID']]).fetchone()
                        if isschk is None:
                            logger.fdebug(
                                "we matched on name, but issue " + arc['IssueNumber'] + " doesn't exist for " + comic[
                                    'ComicName'])
                        else:
                            ...
                            if int(comic['ComicYear']) != int(arc['SeriesYear']):
                                logger.fdebug("Series years are different - discarding match. " + str(
                                    comic['ComicYear']) + " != " + str(arc['SeriesYear']))
                            else:
                                ...
                                break
                if matcheroso == "no":
                    logger.fdebug("[NO WATCHLIST MATCH] Unable to find a match for " + arc['ComicName'] + " :#" + arc[
                        'IssueNumber'])
                    wantedlist.append({
                    ...
                    if filelist is not None and mylar.mylar.CONFIG.STORYARCDIR:
                        logger.fdebug("[NO WATCHLIST MATCH] Checking against lcoal Arc directory for given issue.")
                        fn = 0
                        valids = [x for x in filelist if
                                  re.sub('[\|\s]', '', x['dynamic_name'].lower()).strip() == re.sub('[\|\s]', '', arc[
                                      'DynamicComicName'].lower()).strip()]
                        ...
                        if len(valids) > 0:
                            for tmpfc in valids:  # filelist:
                                ...
                                if int_iss == fcdigit:
                                    logger.fdebug(arc['ComicName'] + ' Issue #' + arc[
                                        'IssueNumber'] + ' already present in StoryArc directory.')
                                    ...
                                    if mylar.mylar.CONFIG.READ2FILENAME:
                                        ...
                                        if all([tmpfc['reading_order'] is not None,
                                                int(readorder) != int(tmpfc['reading_order']['reading_sequence'])]):
                                            logger.warn('reading order sequence has changed for this issue from ' + str(
                                                tmpfc['reading_order']['reading_sequence']) + ' to ' + str(readorder))
                                            rr_rename = True
                                    ...
                                    if rr_rename:
                                        ...
                                        os.rename(os.path.join(tmpfc['comiclocation'], tmpfc['comicfilename']),
                                                  loc_path)
            ...
            logger.fdebug(str(
                len(arc_match)) + " issues currently exist on your watchlist that are within this arc. Analyzing...")
            for m_arc in arc_match:
                ...
                issue = myDB.selectone(
                    ...
                    [m_arc['match_id'], m_arc['match_issue']]).fetchone()
                ...
                if issue is None:
                    ...
                    if issue['Issue_Number'] == m_arc['match_issue']:
                        ...
                        if issue['Status'] == 'Downloaded' or issue['Status'] == 'Archived' or issue[
                            'Status'] == 'Snatched':
                            if showonreadlist:
                            ...
                            if issue['Status'] == 'Downloaded':
                                ...
                                if not os.path.isfile(issloc):
                                    try:
                                        if all([mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS is not None,
                                                mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS != 'None',
                                                os.path.join(mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS,
                                                             os.path.basename(m_arc['match_filedirectory'])) != issloc,
                                                os.path.exists(os.path.join(mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS,
                                                                            os.path.basename(
                                                                                    m_arc['match_filedirectory'])))]):
                                            issloc = os.path.join(mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS,
                                ...
                                if all([mylar.mylar.CONFIG.STORYARCDIR, mylar.mylar.CONFIG.COPY2ARCDIR]):
                                    ...
                                    if mylar.mylar.CONFIG.READ2FILENAME:
                                        ...
                                        if all([m_arc['match_readingorder'] is not None,
                                                int(readorder) != int(m_arc['match_readingorder'])]):
                                            logger.warn('reading order sequence has changed for this issue from ' + str(
                                                m_arc['match_reading_order']) + ' to ' + str(readorder))
                                            rr_rename = True
                                    ...
                                    if not os.path.isfile(dstloc):
                                        ...
                                        try:
                                            ...
                                            logger.error(
                                                'Failed to ' + mylar.mylar.CONFIG.FILE_OPTS + ' ' + issloc + ' - check directories and manually re-run.')
                                            continue
                                    else:
                            ...
                            myDB.upsert("storyarcs", newVal, ctrlVal)
                            logger.info(
                                "Marked " + issue['ComicName'] + " :# " + issue['Issue_Number'] + " as " + issue[
                                    'Status'])
        ...
        if wantedlist is not None:
            for want in wantedlist:
                ...
                if issuechk is None:
                    ...
                    logger.fdebug(u"ComicID: " + str(s_comicid) + " --- IssueID: " + str(
                        s_issueid))  # no comicid in issues table.
                    ...
                    foundcom, prov = search.search_init(ComicName=want['ComicName'], IssueNumber=want['IssueNumber'],
                                                        ComicYear=issueyear, SeriesYear=want['SeriesYear'],
                                                        Publisher=Publisher, IssueDate=issdate, StoreDate=stdate,
                                                        IssueID=s_issueid, SARC=SARC, IssueArcID=IssueArcID,
                                                        oneoff=True)
                else:
                    ...
                    foundcom, prov = search.search_init(ComicName=issuechk['ComicName'],
                                                        IssueNumber=issuechk['Issue_Number'],
                                                        ComicYear=issuechk['IssueYear'],
                                                        SeriesYear=issuechk['SeriesYear'], Publisher=Publisher,
                                                        IssueDate=None, StoreDate=issuechk['ReleaseDate'],
                                                        IssueID=issuechk['IssueID'], AlternateSearch=None,
                                                        UseFuzzy=None, ComicVersion=None, SARC=SARC,
                                                        IssueArcID=IssueArcID)

                if foundcom['status'] is True:
                    logger.fdebug('sucessfully found.')
                    # update the status - this is necessary for torrents as they are in 'snatched' status.
                    updater.foundsearch(s_comicid, s_issueid, mode=mode, provider=prov, SARC=SARC,
                                        IssueArcID=IssueArcID)
                else:
        ...
        if watchlistchk is not None:
            for watchchk in watchlistchk:
                ...
                if issuechk is None:
                    ...
                    foundcom, prov = search.search_init(ComicName=watchchk['ComicName'],
                                                        IssueNumber=watchchk['IssueNumber'], ComicYear=issueyear,
                                                        SeriesYear=watchchk['SeriesYear'], Publisher=None,
                                                        IssueDate=issdate, StoreDate=stdate, IssueID=s_issueid,
                                                        SARC=SARC, IssueArcID=IssueArcID, oneoff=True)
                else:
                    # it's a watched series
                    s_comicid = issuechk['ComicID']
                    s_issueid = issuechk['IssueID']
                    ...
                    foundcom, prov = search.search_init(ComicName=issuechk['ComicName'],
                                                        IssueNumber=issuechk['Issue_Number'],
                                                        ComicYear=issuechk['IssueYear'],
                                                        SeriesYear=issuechk['SeriesYear'], Publisher=None,
                                                        IssueDate=None, StoreDate=issuechk['ReleaseDate'],
                                                        IssueID=issuechk['IssueID'], AlternateSearch=None,
                                                        UseFuzzy=None, ComicVersion=None, SARC=SARC,
                                                        IssueArcID=IssueArcID, mode=None, rsscheck=None, ComicID=None)
                if foundcom['status'] is True:
                    updater.foundsearch(s_comicid, s_issueid, mode=mode, provider=prov, SARC=SARC,
                                        IssueArcID=IssueArcID)
                else:
    ...
    def ReadMassCopy(self, StoryArcID, StoryArcName):
        ...
        if copylist is None:
            ...
            dst = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, StoryArcName)
    ...
    def logs(self):
        return serve_template(templatename="logs.html", title="Log", lineList=mylar.mylar.LOGLIST)

    logs.exposed = True

    def config_dump(self):
        return serve_template(templatename="config_dump.html", title="Config Listing", lineList=mylar.mylar.CONFIG)
    ...
    def toggleVerbose(self):
        if mylar.mylar.LOG_LEVEL != 2:
            mylar.mylar.LOG_LEVEL = 2
        else:
            mylar.mylar.LOG_LEVEL = 1
        if logger.LOG_LANG.startswith('en'):
            logger.initLogger(console=not mylar.mylar.QUIET, log_dir=mylar.mylar.CONFIG.LOG_DIR,
                              max_logsize=mylar.mylar.CONFIG.MAX_LOGSIZE, max_logfiles=mylar.mylar.CONFIG.MAX_LOGFILES,
                              loglevel=mylar.mylar.LOG_LEVEL)
        else:
            logger.mylar_log.stopLogger()
            logger.mylar_log.initLogger(loglevel=mylar.mylar.LOG_LEVEL, log_dir=mylar.mylar.CONFIG.LOG_DIR,
                                        max_logsize=mylar.mylar.CONFIG.MAX_LOGSIZE, max_logfiles=mylar.mylar.CONFIG.MAX_LOGFILES)
        # mylar.mylar.VERBOSE = not mylar.mylar.VERBOSE
        # logger.initLogger(console=not mylar.mylar.QUIET,
        #    log_dir=mylar.mylar.CONFIG.LOG_DIR, verbose=mylar.mylar.VERBOSE)
        if mylar.mylar.LOG_LEVEL == 2:
    ...
    def getLog(self, iDisplayStart=0, iDisplayLength=100, iSortCol_0=0, sSortDir_0="desc", sSearch="", **kwargs):
        ...
        if sSearch == "" or sSearch == None:
            filtered = mylar.mylar.LOGLIST[::]
        else:
            filtered = [row for row in mylar.mylar.LOGLIST for column in row if sSearch.lower() in column.lower()]
        sortcolumn = 0
        if iSortCol_0 == '1':
            sortcolumn = 2
        elif iSortCol_0 == '2':
        ...
        return json.dumps({
            ...
            'iTotalRecords': len(mylar.mylar.LOGLIST),
            'aaData': rows,
        })
    ...
    def getConfig(self, iDisplayStart=0, iDisplayLength=100, iSortCol_0=0, sSortDir_0="desc", sSearch="", **kwargs):
        ...
        iDisplayLength = int(iDisplayLength)
        unfiltered = []
        for each_section in mylar.mylar.config.config.sections():
            for k, v in mylar.mylar.config.config.items(each_section):
    ...
    def downloadLocal(self, IssueID=None, IssueArcID=None, ReadOrder=None, dir=None):
        ...
        if dir is not None:
            ...
            dstPATH = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, issueFILE)
        ...
        if IssueArcID:
            if mylar.mylar.CONFIG.READ2FILENAME:
                ...
                dstPATH = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, ARCissueFILE)
                ISnewValueDict = {'inCacheDIR': 'True',
                                  'Location': issueFILE}
        ...
        #            #print ("comicid: " + str(comicid))
        #            issueLOC = mylar.mylar.CONFIG.DESTINATION_DIR
        #            #print ("IssueLOC: " + str(issueLOC))
        #            issueFILE = issueDL['Location']
        #            #print ("IssueFILE: "+ str(issueFILE))
        #            issuePATH = os.path.join(issueLOC,issueFILE)
    ...
    def MassWeeklyDownload(self, weeknumber=None, year=None, midweek=None, weekfolder=0, filename=None):
        if filename is None:
            mylar.mylar.CONFIG.WEEKFOLDER = bool(int(weekfolder))
            mylar.mylar.CONFIG.writeconfig(values={'weekfolder': mylar.mylar.CONFIG.WEEKFOLDER})
        ...
        if mylar.mylar.CONFIG.WEEKFOLDER:
            if mylar.mylar.CONFIG.WEEKFOLDER_LOC:
                dstdir = mylar.mylar.CONFIG.WEEKFOLDER_LOC
            else:
                dstdir = mylar.mylar.CONFIG.DESTINATION_DIR
            if mylar.mylar.CONFIG.WEEKFOLDER_FORMAT == 0:
                # 0 = YYYY-mm
                desdir = os.path.join(dstdir, str(year) + '-' + str(weeknumber))
            elif mylar.mylar.CONFIG.WEEKFOLDER_FORMAT == 1:
            ...
            desdir = mylar.mylar.CONFIG.GRABBAG_DIR
        ...
        if issuelist is None:  # nothing on the list, just go go gone
            ...
            for issue in issuelist:
                ...
                if issue['Status'] == 'Downloaded':
                    ...
                    logger.info(
                        "Copied " + issue['ComicName'] + " #" + str(issue['Issue_Number']) + " to " + desdir.encode(
                            'utf-8').strip())
                    iscount += 1
    ...
    def confirmResult(self, comicname, comicid):
        ...
        type = 'comic'
        return serve_template(templatename="searchresults.html", title='Import Results for: "' + comicname + '"',
                              searchresults=sresults, type=type, imported='confirm', ogcname=comicid)

    confirmResult.exposed = True

    def Check_ImportStatus(self):
        # logger.info('import_status: ' + mylar.mylar.IMPORT_STATUS)
        return mylar.mylar.IMPORT_STATUS
    ...
    def comicScan(self, path, scan=0, libraryscan=0, redirect=None, autoadd=0, imp_move=0, imp_rename=0, imp_metadata=0,
        ...
        mylar.mylar.CONFIG.ADD_COMICS = autoadd
        # too many problems for windows users, have to rethink this....
        # if 'windows' in mylar.mylar.OS_DETECT.lower() and '\\\\?\\' not in path:
        #    #to handle long paths, let's append the '\\?\' to the path to allow for unicode windows api access
        #    path = "\\\\?\\" + path
        mylar.mylar.CONFIG.COMIC_DIR = path
        mylar.mylar.CONFIG.IMP_MOVE = imp_move
        mylar.mylar.CONFIG.IMP_RENAME = imp_rename
        mylar.mylar.CONFIG.IMP_METADATA = imp_metadata
        # mylar.mylar.config_write()

        logger.info('forcescan is: ' + str(forcescan))
        if mylar.mylar.IMPORTLOCK and forcescan == 1:
            logger.info(
            ...
            mylar.mylar.IMPORTLOCK = False

        # thread the scan.
        if scan == '1':
            scan = True
            mylar.mylar.IMPORT_STATUS = 'Now starting the import'
    ...
    def ThreadcomicScan(self, scan, queue):
        ...
        while True:
            if chk[0]['result'] == 'success':
                ...
                mylar.mylar.IMPORTBUTTON = True  # globally set it to ON after the scan so that it will be picked up.
                mylar.mylar.IMPORT_STATUS = 'Import completed.'
                break
            else:
                yield ckh[0]['result']
                mylar.mylar.IMPORTBUTTON = False
                break
        return
    ...
    def importResults(self):
        ...
        return serve_template(templatename="importresults.html", title="Import Results",
                              results=countit)  # results, watchresults=watchresults)
    ...
    def ImportFilelisting(self, comicname, dynamicname, volume):
        ...
        dynamicname = helpers.conversion(
            urllib.unquote_plus(dynamicname))  # urllib.unquote(dynamicname).decode('utf-8')
        myDB = db.DBConnection()
        if volume is None or volume == 'None':
            results = myDB.select(
    ...
    def deleteimport(self, ComicName, volume, DynamicName, Status):
        ...
        if volume is None or volume == 'None':
            ...
            myDB.action('DELETE from importresults WHERE DynamicName=? AND Volume=? AND Status=?',
                        [DynamicName, volume, Status])
        raise cherrypy.HTTPRedirect("importResults")

    deleteimport.exposed = True

    def preSearchit(self, ComicName, comiclist=None, mimp=0, volume=None, displaycomic=None, comicid=None,
                    dynamicname=None, displayline=None):
        if mylar.mylar.IMPORTLOCK:
            logger.info(
            ...
            for cl in comiclist:
                ...
                for result in results:
                    ...
                    if watchmatched.startswith('C'):
                        ...
                        if mylar.mylar.CONFIG.IMP_MOVE:
                            ...
                            # mylar.mylar.moveit.movefiles(comicid,comloc['ComicLocation'],ComicName)
                            # check for existing files... (this is already called after move files in importer)
                            # updater.forceRescan(comicid)
                        else:
                            raise cherrypy.HTTPRedirect("importResults")
                    else:
                        comicstoIMP.append(result['ComicLocation'])  # .decode(mylar.mylar.SYS_ENCODING, 'replace'))
                        ...
                        if 'annual' in getiss.lower():
                            ...
                            if int(getiss_num) < int(startiss_num):
                                ...
                                if helpers.issuedigits(startISSUE) == 1000 and result[
                                    'ComicYear'] is not None:  # if it's an issue #1, get the year and assume that's the start.
                                    startyear = result['ComicYear']

                # taking this outside of the transaction in an attempt to stop db locking.
                if mylar.mylar.CONFIG.IMP_MOVE and movealreadyonlist == "yes":
                    mylar.mylar.moveit.movefiles(movedata_comicid, movedata_comiclocation, movedata_comicname)
                    updater.forceRescan(comicid)
                    raise cherrypy.HTTPRedirect("importResults")
                ...
                try:
                    if yearRANGE is None:
                        sresults = mb.findComic(searchterm, mode,
                                                issue=numissues)  # ogcname, mode, issue=numissues, explicit='all') #ComicName, mode, issue=numissues)
                    else:
                        sresults = mb.findComic(searchterm, mode, issue=numissues,
                                                limityear=yearRANGE)  # ogcname, mode, issue=numissues, limityear=yearRANGE, explicit='all') #ComicName, mode, issue=numissues, limityear=yearRANGE)
                except TypeError:
                    logger.warn(
    ...
    def importresults_popup(self, SRID, ComicName, imported=None, ogcname=None, DynamicName=None, Volume=None):
        ...
        if not resultset:
            logger.warn('There are no search results to view for this entry ' + ComicName + ' [' + str(
                SRID) + ']. Something is probably wrong.')
            raise cherrypy.HTTPRedirect("importResults")
        ...
        return serve_template(templatename="importresults_popup.html", title="results", searchtext=ComicName,
                              searchresults=searchresults, imported=imported)
...
>>>>>>> Make Mylar launch under Python 3
        ...
        if mylar.mylar.SCHED_RSS_LAST is None:
            ...
            rss_sclast = datetime.datetime.fromtimestamp(mylar.mylar.SCHED_RSS_LAST).replace(microsecond=0)
    ...
    def error_change(self, comicid, errorgcd, comicname, comicyear, imported=None, mogcname=None):
        ...
        if errorgcd[:5].isdigit():
            ...
            self.from_Exceptions(comicid=comicid, gcdid=errorgcd, comicname=cname, comicyear=comicyear,
                                 imported=imported, ogcname=ogcname)
        else:
            logger.info("Assuming rewording of Comic - adjusting to : " + str(errorgcd))
            Err_Info = mylar.mylar.cv.getComic(comicid, 'comic')
            self.addComic(comicid=comicid, comicname=str(errorgcd), comicyear=Err_Info['ComicYear'],
                          comicissues=Err_Info['ComicIssues'], comicpublisher=Err_Info['ComicPublisher'])
...
#--- works, just need to redo search.py to accomodate multiple search terms
    ...
    def readlistOptions(self, send2read=0, tab_enable=0, tab_host=None, tab_user=None, tab_pass=None,
                        tab_directory=None, maintainseriesfolder=0):
        mylar.mylar.CONFIG.SEND2READ = bool(int(send2read))
        mylar.mylar.CONFIG.MAINTAINSERIESFOLDER = bool(int(maintainseriesfolder))
        mylar.mylar.CONFIG.TAB_ENABLE = bool(int(tab_enable))
        mylar.mylar.CONFIG.TAB_HOST = tab_host
        mylar.mylar.CONFIG.TAB_USER = tab_user
        mylar.mylar.CONFIG.TAB_PASS = tab_pass
        mylar.mylar.CONFIG.TAB_DIRECTORY = tab_directory
        readoptions = {'send2read': mylar.mylar.CONFIG.SEND2READ,
                       'maintainseriesfolder': mylar.mylar.CONFIG.MAINTAINSERIESFOLDER,
                       'tab_enable': mylar.mylar.CONFIG.TAB_ENABLE,
                       'tab_host': mylar.mylar.CONFIG.TAB_HOST,
                       'tab_user': mylar.mylar.CONFIG.TAB_USER,
                       'tab_pass': mylar.mylar.CONFIG.TAB_PASS,
                       'tab_directory': mylar.mylar.CONFIG.TAB_DIRECTORY}
        mylar.mylar.CONFIG.writeconfig(values=readoptions)

        raise cherrypy.HTTPRedirect("readlist")

    readlistOptions.exposed = True

    def arcOptions(self, StoryArcID=None, StoryArcName=None, read2filename=0, storyarcdir=0, arc_folderformat=None,
                   copy2arcdir=0, arc_fileops='copy'):
        mylar.mylar.CONFIG.READ2FILENAME = bool(int(read2filename))
        mylar.mylar.CONFIG.STORYARCDIR = bool(int(storyarcdir))
        if arc_folderformat is None:
            mylar.mylar.CONFIG.ARC_FOLDERFORMAT = "($arc) ($spanyears)"
        else:
            mylar.mylar.CONFIG.ARC_FOLDERFORMAT = arc_folderformat
        mylar.mylar.CONFIG.COPY2ARCDIR = bool(int(copy2arcdir))
        mylar.mylar.CONFIG.ARC_FILEOPS = arc_fileops
        options = {'read2filename': mylar.mylar.CONFIG.READ2FILENAME,
                   'storyarcdir': mylar.mylar.CONFIG.STORYARCDIR,
                   'arc_folderformat': mylar.mylar.CONFIG.ARC_FOLDERFORMAT,
                   'copy2arcdir': mylar.mylar.CONFIG.COPY2ARCDIR,
                   'arc_fileops': mylar.mylar.CONFIG.ARC_FILEOPS}
        mylar.mylar.CONFIG.writeconfig(values=options)

        # force the check/creation of directory com_location here
        if mylar.mylar.CONFIG.STORYARCDIR is True:
            arcdir = os.path.join(mylar.mylar.CONFIG.DESTINATION_DIR, 'StoryArcs')
    ...
    def configUpdate(self, **kwargs):
        checked_configs = ['enable_https', 'launch_browser', 'syno_fix', 'auto_update', 'annuals_on', 'api_enabled',
                           'nzb_startup_search',
                           'enforce_perms', 'sab_to_mylar', 'torrent_local', 'torrent_seedbox', 'rtorrent_ssl',
                           'rtorrent_verify', 'rtorrent_startonload',
...
=======
                           'dognzb', 'dognzb_verify', 'experimental', 'enable_torrent_search', 'enable_public',
                           'enable_32p', 'enable_torznab',
                           'newznab', 'use_minsize', 'use_maxsize', 'ddump', 'failed_download_handling',
                           'sab_client_post_processing', 'nzbget_client_post_processing',
                           'failed_auto', 'post_processing', 'enable_check_folder', 'enable_pre_scripts',
                           'enable_snatch_script', 'enable_extra_scripts',
                           'enable_meta', 'cbr2cbz_only', 'ct_tag_cr', 'ct_tag_cbl', 'ct_cbz_overwrite', 'rename_files',
                           'replace_spaces', 'zero_level',
                           'lowercase_filenames', 'autowant_upcoming', 'autowant_all', 'comic_cover_local',
                           'alternate_latest_series_covers', 'cvinfo', 'snatchedtorrent_notify',
                           'prowl_enabled', 'prowl_onsnatch', 'nma_enabled', 'nma_onsnatch', 'pushover_enabled',
                           'pushover_onsnatch', 'boxcar_enabled',
                           'boxcar_onsnatch', 'pushbullet_enabled', 'pushbullet_onsnatch', 'telegram_enabled',
                           'telegram_onsnatch', 'slack_enabled', 'slack_onsnatch',
...
>>>>>>> Make Mylar launch under Python 3
        ...
        for k, v in kwargs.iteritems():
            try:
                _conf = mylar.mylar.CONFIG._define(k)
            except KeyError:
                continue

        mylar.mylar.CONFIG.EXTRA_NEWZNABS = []
        ...
        for kwarg in [x for x in kwargs if x.startswith('newznab_name')]:
            if kwarg.startswith('newznab_name'):
                ...
                mylar.mylar.CONFIG.EXTRA_NEWZNABS.append(
                    (newznab_name, newznab_host, newznab_verify, newznab_api, newznab_uid, newznab_enabled))

        mylar.mylar.CONFIG.EXTRA_TORZNABS = []
        ...
        for kwarg in [x for x in kwargs if x.startswith('torznab_name')]:
            if kwarg.startswith('torznab_name'):
                ...
                mylar.mylar.CONFIG.EXTRA_TORZNABS.append(
                    (torznab_name, torznab_host, torznab_api, torznab_category, torznab_enabled))

        mylar.mylar.CONFIG.process_kwargs(kwargs)

        # this makes sure things are set to the default values if they're not appropriately set.
        mylar.mylar.CONFIG.configure(update=True)
    ...
    def SABtest(self, sabhost=None, sabusername=None, sabpassword=None, sabapikey=None):
        if sabhost is None:
            sabhost = mylar.mylar.CONFIG.SAB_HOST
        if sabusername is None:
            sabusername = mylar.mylar.CONFIG.SAB_USERNAME
        if sabpassword is None:
            sabpassword = mylar.mylar.CONFIG.SAB_PASSWORD
        if sabapikey is None:
            sabapikey = mylar.mylar.CONFIG.SAB_APIKEY
    ...
    def NZBGet_test(self, nzbhost=None, nzbport=None, nzbusername=None, nzbpassword=None):
        if nzbhost is None:
            nzbhost = mylar.mylar.CONFIG.NZBGET_HOST
        if nzbport is None:
            nzbport = mylar.mylar.CONFIG.NZBGET_PORT
        if nzbusername is None:
            nzbusername = mylar.mylar.CONFIG.NZBGET_USERNAME
        if nzbpassword is None:
            nzbpassword = mylar.mylar.CONFIG.NZBGET_PASSWORD
    ...
    def getInfo(self, ComicID=None, IssueID=None):
        ...
        from mylar.mylar.mylar import cache
    ...
    def findsabAPI(self, sabhost=None, sabusername=None, sabpassword=None):
        ...
        sabapi = sp.sab_get()
        logger.info(
            'SAB APIKey found as : ' + str(sabapi) + '. You still have to save the config to retain this setting.')
        mylar.mylar.CONFIG.SAB_APIKEY = sabapi
        return sabapi
    ...
    def generateAPI(self):
        ...
        mylar.mylar.CONFIG.API_KEY = apikey
        return apikey
    ...
    def api(self, *args, **kwargs):
        ...
        from mylar.mylar.api import Api
    ...
    def opds(self, *args, **kwargs):
        from mylar.mylar.opds import OPDS
    ...
    def IssueInfo(self, filelocation, comicname=None, issue=None, date=None, title=None):
        ...
        if issuedetails:
            ...
            issueinfo += '<img style="float: left; padding-right: 10px" src=' + issuedetails[0][
                'IssueImage'] + ' height="400" width="263">'
            ...
            if all([issuedetails[0]['day'] is None, issuedetails[0]['month'] is None, issuedetails[0]['year'] is None]):
                ...
                issueinfo += '<p class="alignright">(' + str(issuedetails[0]['year']) + '-' + str(
                    issuedetails[0]['month']) + '-' + str(issuedetails[0]['day']) + ')</p></br>'
    ...
    def manual_metatag(self, dirName, issueid, filename, comicid, comversion, seriesyear=None, group=False):
        ...
        try:
            ...
            if mylar.mylar.CONFIG.CMTAG_START_YEAR_AS_VOLUME:
            ...
            metaresponse = cmtagmylar.mylar.run(dirName, issueid=issueid, filename=filename, comversion=vol_label,
                                          manualmeta=True)
        except ImportError:
            logger.warn(
                module + ' comictaggerlib not found on system. Ensure the ENTIRE lib directory is located within mylar/lib/comictaggerlib/ directory.')
            metaresponse = "fail"

        if metaresponse == "fail":
            logger.fdebug(module + ' Unable to write metadata successfully - check mylar.mylar.log file.')
            return
        elif metaresponse == "unrar error":
            logger.error(
                module + ' This is a corrupt archive - whether CRC errors or it is incomplete. Marking as BAD, and retrying a different copy.')
            return
            ...
            try:
                ...
                logger.info('%s Sucessfully wrote metadata to .cbz (%s) - Continuing..' % (
                module, os.path.split(metaresponse)[1]))
            except Exception as e:
                if str(e.errno) == '2':
                    try:
                        if mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS is not None and mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS != 'None' and os.path.join(
                                mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS, os.path.basename(dirName)) != dirName:
                            dst = os.path.join(mylar.mylar.CONFIG.MULTIPLE_DEST_DIRS, os.path.basename(dirName))
                            shutil.copy(metaresponse, dst)
                            logger.info('%s Sucessfully wrote metadata to .cbz (%s) - Continuing..' % (
                            module, os.path.split(metaresponse)[1]))
                    except Exception as e:
            ...
            if filename is not None:
                if os.path.isfile(filename) and os.path.split(filename)[1].lower() != os.path.split(metaresponse)[
                    1].lower():
                    try:
    ...
    def group_metatag(self, ComicID, dirName=None):
        myDB = db.DBConnection()
        cinfo = myDB.selectone('SELECT ComicLocation, ComicVersion, ComicYear, ComicName FROM comics WHERE ComicID=?',
                               [ComicID]).fetchone()
        ...
        for ginfo in groupinfo:
            ...
            self.manual_metatag(meta_dir, ginfo['IssueID'], os.path.join(meta_dir, ginfo['Location']), ComicID,
                                comversion=cinfo['ComicVersion'], seriesyear=cinfo['ComicYear'], group=True)
        updater.forceRescan(ComicID)
        logger.info('[SERIES-METATAGGER][' + cinfo['ComicName'] + ' (' + cinfo[
            'ComicYear'] + ')] Finished doing a complete series (re)tagging of metadata.')

    group_metatag.exposed = True

    def CreateFolders(self, createfolders=None):
        if createfolders:
            mylar.mylar.CONFIG.CREATE_FOLDERS = int(createfolders)
            # mylar.mylar.config_write()
    ...
    def search_32p(self, search=None):
        return mylar.mylar.rsscheck.torrents(pickfeed='4', seriesname=search)
    ...
    def testrtorrent(self, host, username, password, auth, verify, ssl, rpc_url):
        from mylar.mylar.mylar.mylar.torrent.clients import rtorrent as TorClient
        client = TorClient.TorrentClient()
        ca_bundle = None
        if mylar.mylar.CONFIG.RTORRENT_CA_BUNDLE is not None:
            ca_bundle = mylar.mylar.CONFIG.RTORRENT_CA_BUNDLE
    ...
    def torrentit(self, issueid=None, torrent_hash=None, download=False):
        ...
        if mylar.mylar.CONFIG.AUTO_SNATCH is False:
            logger.warn(
        ...
        if torrent_info:
            ...
            logger.info("Client: %s", mylar.mylar.CONFIG.RTORRENT_HOST)
            ...
            if torrent_info['completed'] and download is True:
                ti += '<br><center><tr><td>AUTO-SNATCH ENABLED: ' + torrent_info[
                    'snatch_status'] + '</center></td></tr>'
            ti += '<tr><td><center>Hash: ' + torrent_info['hash'] + '</center></td></tr>'
            ti += '<tr><td><center>Location: ' + os.path.join(torrent_info['folder'],
                                                              torrent_name) + '</center></td></tr></br>'
            ti += '<tr><td><center>Filesize: ' + torrent_info['filesize'] + '</center></td></tr>'
            ti += '<tr><td><center>' + torrent_info['download'] + ' DOWN / ' + torrent_info[
                'upload'] + ' UP</center></td></tr>'
            ...
            # fp = mylar.mylar.process.Process(torrent_info['filepath'], torrent_info['dst_folder'], issueid=torrent_info['issueid'], failed=failed)
            # fp.post_process()
    ...
    def download_0day(self, week):
        ...
        foundcom, prov = search.search_init('0-Day Comics Pack - %s.%s' % (week[:4], week[5:]), None, week[:4], None,
                                            None, week, week, None, allow_packs=True, oneoff=True)
    ...
    def test_32p(self):
        ...
        if rtnvalues['status'] is True:
            return json.dumps({"status": "Successfully Authenticated.", "inkdrops": mylar.mylar.INKDROPS_32P})
        else:
            return json.dumps({"status": "Could not Authenticate.", "inkdrops": mylar.mylar.INKDROPS_32P})
        ...
        if weeknumber is not None:
            ...
            for weekly in w_results:
                ...
                if weekly['ComicID'] in watchlibrary:
                    ...
                    if all([mylar.mylar.CONFIG.AUTOWANT_UPCOMING, tmp_status == 'Skipped']):
                        tmp_status = 'Wanted'
            ...
            for ab in weeklylist:
                if ab['HAVEIT'] == ab['COMICID']:
                    lb = myDB.selectone('SELECT ComicVersion, Type, ComicYear from comics WHERE ComicID=?',
                                        [ab['COMICID']]).fetchone()
    ...
    def downloadBanner(self, **kwargs):
        ...
        for i in ['.jpg', '.png', '.jpeg']:
            if i in url:
                ext = i
                break
        banner = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, 'storyarcs', (str(storyarcid) + '-banner' + ext))
    ...
    def manageBanner(self, comicid, action, height=None, width=None):
        rootpath = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, 'storyarcs')
        if action == 'delete':
            delete = False
            ext = ['.jpg', '.png', '.jpeg']
            ...
            # ext = ['.jpg', '.png', '.jpeg']
            # ext = None
            # loc = os.path.join(mylar.mylar.CONFIG.CACHE_DIR, 'storyarcs', str(comicid) + '-banner')
            # for x in ['.jpg', '.png', '.jpeg']:
            #    if os.path.isfile(loc +x):
            #        ext = x
            #        break
            if filepath is not None:
                ...
                logger.info('successfully saved %s to new dimensions of banner : 960 x %s' % (
                str(comicid) + '-bannerH' + str(height) + ext, height))
            else:
    ...
    def choose_specific_download(self, **kwargs):  # manual=True):
        ...
        if all([kwargs['issueid'] != 'None', kwargs['issueid'] is not None]) and action is False:
            ...
            results = self.queueissue(kwargs['mode'], ComicName=kwargs['comicname'], ComicID=kwargs['comicid'],
                                      IssueID=kwargs['issueid'], ComicIssue=kwargs['issue'], ComicVersion=comicvolume,
                                      Publisher=kwargs['publisher'], pullinfo=kwargs['pullinfo'],
                                      pullweek=kwargs['pullweek'], pullyear=kwargs['pullyear'], manual=True)

        myDB = db.DBConnection()
        r = []

        for x in mylar.mylar.COMICINFO:  # results:
    ...
    def download_specific_release(self, nzbid, provider, name):
        myDB = db.DBConnection()
        dsr = myDB.selectone('SELECT * FROM manualresults WHERE id=? AND tmpprov=? AND name=?',
                             [nzbid, provider, name]).fetchone()
        if dsr is None:
        ...
        try:
            ...
            sresults = search.searcher(dsr['fullprov'], nzbname, comicinfo, link=link, IssueID=dsr['issueid'],
                                       ComicID=dsr['comicid'], tmpprov=dsr['tmpprov'], directsend=True,
                                       newznab=newznabinfo)
            if sresults is not None:
                updater.foundsearch(dsr['ComicID'], dsr['IssueID'], mode='series', provider=dsr['tmpprov'],
                                    hash=sresults['t_hash'])
        except:
