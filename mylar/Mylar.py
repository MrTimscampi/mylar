...
def main():
    ...
    if hasattr(sys, 'frozen'):
        mylar.mylar.FULL_PATH = os.path.abspath(sys.executable)
    else:
        mylar.mylar.FULL_PATH = os.path.abspath(__file__)

    mylar.mylar.PROG_DIR = os.path.dirname(mylar.mylar.FULL_PATH)
    mylar.mylar.ARGS = sys.argv[1:]

    # From sickbeard
    mylar.mylar.SYS_ENCODING = None

    try:
        locale.setlocale(locale.LC_ALL, "")
        mylar.mylar.SYS_ENCODING = locale.getpreferredencoding()
    except (locale.Error, IOError):
        pass

    # for OSes that are poorly configured I'll just force UTF-8
    if not mylar.mylar.SYS_ENCODING or mylar.mylar.SYS_ENCODING in ('ANSI_X3.4-1968', 'US-ASCII', 'ASCII'):
        mylar.mylar.SYS_ENCODING = 'UTF-8'
    ...
    if args.verbose:
        ...
        mylar.mylar.QUIET = True
        print('Quiet logging mode enabled...')
        mylar.mylar.LOG_LEVEL = 0
    else:
    ...
    if args.daemon:
        if sys.platform == 'win32':
            ...
            mylar.mylar.DAEMON = True

    if args.pidfile:
        mylar.mylar.PIDFILE = str(args.pidfile)

        # If the pidfile already exists, mylar may still be running, so exit
        if os.path.exists(mylar.mylar.PIDFILE):
            sys.exit("PID file '" + mylar.mylar.PIDFILE + "' already exists. Exiting.")

        # The pidfile is only useful in daemon mode, make sure we can write the file properly
        if mylar.mylar.DAEMON:
            mylar.mylar.CREATEPID = True
            try:
                file(mylar.mylar.PIDFILE, 'w').write("pid\n")
            except IOError as e:
    ...
    if args.datadir:
        mylar.mylar.DATA_DIR = args.datadir
    else:
        mylar.mylar.DATA_DIR = mylar.mylar.PROG_DIR

    if args.config:
        mylar.mylar.CONFIG_FILE = args.config
    else:
        mylar.mylar.CONFIG_FILE = os.path.join(mylar.mylar.DATA_DIR, 'config.ini')

    if args.safe:
        mylar.mylar.SAFESTART = True
    else:
        mylar.mylar.SAFESTART = False

    if args.noweekly:
        mylar.mylar.NOWEEKLY = True
    else:
        mylar.mylar.NOWEEKLY = False

    # Put the database in the DATA_DIR
    mylar.mylar.DB_FILE = os.path.join(mylar.mylar.DATA_DIR, 'mylar.db')
    ...
    #try:
    mylar.mylar.initialize(mylar.mylar.CONFIG_FILE)
    #except Exception as e:
    #    print e
    #    raise SystemExit('FATAL ERROR')

    if mylar.mylar.MAINTENANCE is False:
        filechecker.validateAndCreateDirectory(mylar.mylar.DATA_DIR, True)

        # Make sure the DATA_DIR is writeable
        if not os.access(mylar.mylar.DATA_DIR, os.W_OK):
            raise SystemExit('Cannot write to the data directory: ' + mylar.mylar.DATA_DIR + '. Exiting...')

    # backup the db and configs before they load.
    if args.backup:
        print('[AUTO-BACKUP] Backing up .db and config.ini files for safety.')
        backupdir = os.path.join(mylar.mylar.DATA_DIR, 'backup')
    ...
    if mylar.MAINTENANCE is True and any([args.exportjson, args.importjson, args.update is True, args.importstatus is True, args.fixslashes is True]):
        ...
        if args.importstatus: #mylar.MAINTENANCE is True:
            ...
            if args.update is True:
                ...
                try:
                    mylar.mylar.shutdown(restart=True, update=True, maintenance=True)
                except Exception as e:
    ...
    if args.port:
        ...
        http_port = int(mylar.mylar.CONFIG.HTTP_PORT)

    # Check if pyOpenSSL is installed. It is required for certificate generation
    # and for CherryPy.
    if mylar.mylar.CONFIG.ENABLE_HTTPS:
        try:
            import OpenSSL
        except ImportError:
            logger.warn("The pyOpenSSL module is missing. Install this " \
                "module to enable HTTPS. HTTPS will be disabled.")
            mylar.mylar.CONFIG.ENABLE_HTTPS = False

    # Try to start the server. Will exit here is address is already in use.
    web_config = {
        'http_port': http_port,
        'http_host': mylar.mylar.CONFIG.HTTP_HOST,
        'http_root': mylar.mylar.CONFIG.HTTP_ROOT,
        'enable_https': mylar.mylar.CONFIG.ENABLE_HTTPS,
        'https_cert': mylar.mylar.CONFIG.HTTPS_CERT,
        'https_key': mylar.mylar.CONFIG.HTTPS_KEY,
        'https_chain': mylar.mylar.CONFIG.HTTPS_CHAIN,
        'http_username': mylar.mylar.CONFIG.HTTP_USERNAME,
        'http_password': mylar.mylar.CONFIG.HTTP_PASSWORD,
        'authentication': mylar.mylar.CONFIG.AUTHENTICATION,
        'login_timeout': mylar.mylar.CONFIG.LOGIN_TIMEOUT,
        'opds_enable': mylar.mylar.CONFIG.OPDS_ENABLE,
        'opds_authentication': mylar.mylar.CONFIG.OPDS_AUTHENTICATION,
        'opds_username': mylar.mylar.CONFIG.OPDS_USERNAME,
        'opds_password': mylar.mylar.CONFIG.OPDS_PASSWORD,
    }
    ...
    if mylar.mylar.CONFIG.LAUNCH_BROWSER and not args.nolaunch:
        mylar.mylar.launch_browser(mylar.mylar.CONFIG.HTTP_HOST, http_port, mylar.mylar.CONFIG.HTTP_ROOT)
    ...
    while True:
        if not mylar.mylar.SIGNAL:
            try:
                time.sleep(1)
            except KeyboardInterrupt:
                mylar.mylar.SIGNAL = 'shutdown'
        else:
            logger.info('Received signal: ' + mylar.mylar.SIGNAL)
            if mylar.mylar.SIGNAL == 'shutdown':
                mylar.mylar.shutdown()
            elif mylar.mylar.SIGNAL == 'restart':
                mylar.mylar.shutdown(restart=True)
            else:
                mylar.mylar.shutdown(restart=True, update=True)

            mylar.mylar.SIGNAL = None
